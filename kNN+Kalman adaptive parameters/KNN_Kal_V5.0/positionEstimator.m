...
%   newModelParameters - Updated model parameters after processing test_data.
    ...
    num_neurons = length(test_data.spikes(:,1));
    ...
    if fla
        ...
        direction = mode(customPredictKNN(modes, spike_count));
        % Update the direction in the model parameters
        modelParameters(1).direction = direction;
        % Previous position
        xl = test_data.startHandPos(1);
        yl = test_data.startHandPos(2);
    else
        % Use the previously predicted direction if beyond initial 320ms
        direction = modelParameters(1).direction;
        xl = test_data.decodedHandPos(1,end);
        yl = test_data.decodedHandPos(2,end);
    end
    
    %% Firing Rate Calculation
    % Initialize array for storing firing rates of all neurons
    total_firing_rate = zeros(1,num_neurons);
    % Calculate firing rate for each neuron in the specified time window
    for i = 1:num_neurons
        total_spikes = sum(test_data.spikes(i,tmin:tmax));
        total_firing_rate(i) = total_spikes / (dt*0.001); % Normalize by the window duration
    end
    curtime = modelParameters(1).timeBin(direction);
    if t_fla >= curtime
        t_fla = curtime - 1 ;
    end
    ...
    [state, newModelParameters] = kalmanFilter(test_data, total_firing_rate, modelParameters, direction, fla, t_fla); 
    xpre = state(1);
    ypre = state(2);
    if fla
        x = xpre;

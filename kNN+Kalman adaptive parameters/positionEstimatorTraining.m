    ...
    for t = 1:twin
        ...
        for neuron = 1:nneurons
            ...
            for n = 1:ntrial
                ...
                total_spikes = sum(training_data(n,angle).spikes(neuron,t_map:t_map+binsize));
                store_sr = cat(2, store_sr, total_spikes/(binsize*0.001));
                
                % Calculate velocities and positions if first neuron (done once per trial)
                if neuron == 1
                    x1 = training_data(n,angle).handPos(1,t_map) / 100;
                    x2 = training_data(n,angle).handPos(1,t_map+binsize)/ 100;
                    y1 = training_data(n,angle).handPos(2,t_map)/ 100;
                    y2 = training_data(n,angle).handPos(2,t_map+binsize)/ 100;
...
%% KNN Classifier Training
    ...
    for angle = 1:ndire
        for neuron = 1:nneurons
            for n = 1:ntrial
                total_spikes = sum(training_data(n,angle).spikes(neuron, 1:320));

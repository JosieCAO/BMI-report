...
%   newModelParameters - Updated model parameters after processing test_data.
    ...
    if fla <= 1
        ...
        spike_count = sum(test_data.spikes(:,50:320),2)';
    ...
    for i = 1:num_neurons
        ...
        total_firing_rate(i) = total_spikes / dt; % Normalize by the window duration
    end
    
    %% Kalman Filter Update
    % Update the hand position estimates using the Kalman filter
    [x, y, newModelParameters] = ExtKalmanFilter(test_data, total_firing_rate, modelParameters, direction, fla); 
